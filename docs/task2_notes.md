...
当前 pipeline 的核心思路是：先把原始过车记录聚合到 20 分钟粒度，再用观测窗口、日历、天气和历史统计特征预测目标窗口流量。
...
- `src/kddcup2017_task2/features.py`：构造日历、天气、观测窗口、历史均值和 lag 特征
...
- 当前默认 ExtraTrees + global + log：MAPE 约 `0.130091`
